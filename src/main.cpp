...
void runSimulation(std::map<std::string, OrderBook>& books,
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (doCancel) {
            std::uniform_int_distribution<size_t> pickDist(0, liveIds.size() - 1);
            size_t idx = pickDist(rng);
            int targetId = liveIds[idx];
            liveIds[idx] = liveIds.back();
            liveIds.pop_back();

            auto t1 = std::chrono::steady_clock::now();
            bool ok = book.cancelOrder(targetId);
...
void runMultiThreadSim(const std::string& symbol, int count,
    ...
    latency.report("Multi-Thread addOrder Latency");
}
